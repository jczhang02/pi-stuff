...
await runEffect(async () => {
  ...
  const sections = scenes.map(scene => {
    const images = files
      .filter(
        file => file.startsWith(scene.name + '-') && file.endsWith('.png'),
      )
      .filter(
        file =>
...
</style><h1>Pi Stuff · 会话原型</h1><p>以当前功能为范围, 在真实 Pi 中运行的新会话样式. 这里的图片来自可交互终端, 模型与执行数据是离线样例. <a href="README.md">启动与操作说明</a> · <a href="../../docs/i18n/zh-CN/research/ui-session-prototype-2026-09-21.md">来源与详细规则</a> · <a href="../ui-direction/gallery.html">上一轮方案</a></p><ul><li>Read/Grep/Find/Ls 连续成功两次起聚合. 点击组查看调用, 再点一项查看内容.</li><li>工具采用“操作与目标 / 结果”两层. Write、Web 和成功 Bash 默认收起, Edit 保留三行短 diff.</li><li>WebSearch / WebFetch / WebRead 共用相同样式; 元数据只在展开后显示.</li><li>Thoughts 显示耗时, 点击阅读正文. 回答失败或中断保持普通文字.</li></ul><nav>${nav}</nav>${sections.join('')}<footer><p>点击工具或 Thoughts 展开, Ctrl+O 切换全部详情. replay 场景可按 Esc 中断. 完整会话用 Pi 原生滚动查看. 截图使用 Catppuccin 与指定 Nerd Font 字体栈; 尚未生产采纳.</p></footer></html>`,
  );
});
