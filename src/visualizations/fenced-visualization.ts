import {renderTreeSource} from './indentation-tree';
import {renderChartSource} from './unicode-chart';
...
export function prepareFencedVisualizations(
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    const rendered = sourceIsSafe(source)
      ? renderVisualization(language, source, width, measureWidth)
      : [];
    if (rendered.length === 0) {
      output.push(...lines.slice(index, close + 1));
    } else {
      output.push(
        ...markdownCodeBlock(rendered, opening.indentation, projectionLanguage),
      );
      projectedBlocks.push({
        firstLine: rendered[0] ?? '',
...
function sourceLengthExceedsLimit(
...
function visualizationLanguage(
  value: string,
...
function renderVisualization(
  ...
  return language === 'chart'
    ? renderChartSource(source, width)
...
function parseOpeningFence(line: string): OpeningFence | undefined {
  ...
  const character: FenceCharacter =
    marker.charCodeAt(0) === 0x60 ? 'backtick' : 'tilde';
...
function selectProjectionLanguage(
  lines: readonly string[],
): string | undefined {
  ...
  for (let variant = 0; variant <= MAX_LANGUAGE_VARIANT; variant += 1) {
    if (occupied[variant] === 0) {
      return variant === 0
        ? VISUALIZATION_CODE_LANGUAGE
        : `${VISUALIZATION_CODE_LANGUAGE}-${String(variant)}`;
    }
  }
  return undefined;
}
...
function markdownCodeBlock(
  ...
  let longestRun = 2;
  for (const line of lines) {
    for (const match of line.matchAll(/\x60+/gu))
