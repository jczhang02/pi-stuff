 ...
 * Copyright (c) 2026 Howaboua. See LICENSE-Howaboua.txt.
 * Source commit: 8d63d300597488e6fa4c30ccd6a3eb0fed2d4304.
 */
...
import {visibleWidth as measureWidth} from '@earendil-works/pi-tui';
...
function parseChartSource(source: string): ChartSpec | undefined {
  ...
  if (chartType === 'heatmap') {
    if (dataLines.length === 0 || dataLines.length > MAX_HEATMAP_ROWS)
      return undefined;
  ...
  if (chartType === 'sparkline') {
    ...
    return points.length > 0
      ? createChartSpec(chartType, points, title)
      : undefined;
  }
...
function parseNumber(value: string | undefined): number | undefined {
  ...
  return Number.isFinite(number) && Math.abs(number) <= Number.MAX_VALUE / 128
    ? number
    : undefined;
}
...
function renderBars(points: ChartPoint[], width: number): string[] {
  ...
  const yLabelWidth = Math.max(
    3,
    ...tickLabels.map(label => measureWidth(label)),
  );
...
function renderSparkline(points: ChartPoint[], width: number): string[] {
  ...
  const chartWidth = Math.max(
    4,
    width - measureWidth(minimumLabel) - measureWidth(maximumLabel) - 2,
  );
...
function renderBraille(
  ...
  const spacer = Math.max(
    1,
    plotColumns - measureWidth(first) - measureWidth(last),
  );
  lines.push(
...
function renderHeatmap(rows: HeatmapRow[], width: number): string[] {
  const rowLabelWidth = Math.min(
    14,
    Math.max(3, ...rows.map(row => measureWidth(row.label))),
  );
