...
/**
 * Single source of truth for the Pi Stuff permissions config file shape.
 *
 ...
 *    `buildPermissionsJsonSchema()` and verified by the repository test suite.
 *
...
export const PERMISSIONS_SCHEMA_URL =
	"https://raw.githubusercontent.com/jczhang02/pi-stuff/main/packages/pi-stuff-permissions/schemas/permissions.schema.json";
...
const patternValueSchema = z.union([permissionStateSchema, denyWithReasonSchema]);
...
const shellToolsSchema = z
	...
	.meta({
		...
		markdownDescription:
			'Records which non-`bash` tools carry shell semantics, mapping each tool name to the input argument holding its command (and optionally its working directory).\n\nUse this when an extension replaces Pi\'s native `bash` tool under another name — for example, an `exec_command` tool with a `cmd` argument and optional `workdir`. The destructive-command circuit breaker and any manual shell policy then inspect it exactly like native `bash`.\n\nExample:\n\n```json\n"shellTools": {\n  "exec_command": { "commandArgument": "cmd", "workdirArgument": "workdir" }\n}\n```\n\nThis map is user-owned global configuration. Project content cannot enroll a shell alias or replace an existing mapping.',
		examples: [
			{
				exec_command: { commandArgument: "cmd", workdirArgument: "workdir" },
			},
...
export const unifiedConfigSchema = z
	.strictObject({
		...
		permissionMode: z.enum(["unrestricted", "manual"]).optional().meta({
			description: "Default permission posture. Destructive tripwires remain active in both modes.",
		...
		debugLog: z.boolean().optional().meta({
			...
			markdownDescription:
				"Write verbose diagnostics to `logs/pi-stuff-permissions-debug.jsonl` under the extension config directory.",
			default: false,
		}),
		permissionReviewLog: z.boolean().optional().meta({
			...
			markdownDescription:
				"Write permission request and decision audit events to `logs/pi-stuff-permissions-permission-review.jsonl` under the extension config directory.",
			default: false,
		}),
		yoloMode: z.boolean().optional().meta({
			description: "Auto-approve ordinary manual-policy asks. Destructive-command tripwires remain active.",
			markdownDescription:
				"Auto-approve ordinary `ask`-state policy checks, including subagent forwarding.\n\nThe non-relaxable destructive-command circuit breaker still denies protected targets and still asks for an exact recoverable outside-project deletion or Git discard.",
			default: false,
		}),
	...
	.meta({
		title: "Pi Stuff Permissions Configuration",
		description: "Runtime settings and optional manual permission policy for Pi Stuff.",
		markdownDescription:
			"Pi Stuff permission settings. Place global user-owned runtime controls at `~/.pi/agent/extensions/pi-stuff-permissions/config.json`. Project policy may tighten ordinary permissions but cannot weaken the destructive-command circuit breaker.",
	});
