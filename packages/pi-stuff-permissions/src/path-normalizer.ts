...
export class PathNormalizer {
	...
	isOutsideWorkingDirectory(pathValue: string): boolean {
		const canonicalPath = canonicalNormalizePathForComparison(pathValue, this.cwd, this.flavor);
		return isPathOutsideWorkingDirectory(canonicalPath, this.canonicalCwd, this.flavor);
	}
	 ...
	 */
	isInfrastructureRead(toolName: string, accessPath: AccessPath, infraDirs: readonly string[]): boolean {
		return isPiInfrastructureRead(toolName, accessPath.boundaryValue(), infraDirs, this.cwd, this.flavor);
	}
