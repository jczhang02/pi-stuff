...
/**
 * Narrow subset of `ConfigStore` for the `/permissions` command.
 *
 * Using an interface rather than the concrete class avoids private-member
 * coupling between the class and test doubles.
 */
...
export class ConfigStore implements SessionConfigStore, CommandConfigStore {
	...
	refresh(ctx: ExtensionContext | undefined, projectTrusted: boolean): void {
		...
		const userConfig = normalizePermissionSystemConfig(mergeResult.global);
		// Runtime authority and logging are user-owned. A trusted project may
		// tighten policy, but it cannot turn on yolo/external authorizers or cause
		// raw commands to be written to logs.
		const runtimeConfig: PermissionSystemExtensionConfig = {
			...mergedConfig,
			permissionMode: userConfig.permissionMode,
			debugLog: userConfig.debugLog,
			permissionReviewLog: userConfig.permissionReviewLog,
			yoloMode: userConfig.yoloMode,
			doublePressToConfirm: userConfig.doublePressToConfirm,
			...(userConfig.shellTools ? { shellTools: userConfig.shellTools } : {}),
			...(userConfig.authorizerChain ? { authorizerChain: userConfig.authorizerChain } : {}),
		};
		if (!userConfig.authorizerChain) {
			Reflect.deleteProperty(runtimeConfig, "authorizerChain");
		}
		if (!userConfig.shellTools) {
			Reflect.deleteProperty(runtimeConfig, "shellTools");
		}
	...
	save(next: PermissionSystemExtensionConfig, ctx: ExtensionCommandContext): void {
		...
		try {
			...
			ctx.ui.notify(`Failed to save Pi Stuff permissions at '${globalPath}': ${message}`, "error");
			return;
		}
