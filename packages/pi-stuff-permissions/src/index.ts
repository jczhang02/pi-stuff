...
import { isManualPermissionMode, isYoloModeEnabled } from "./extension-config";
...
export default function piStuffPermissions(pi: ExtensionAPI): void {
	...
	const permissionManager = new PermissionManager({
		...
		isManualModeEnabled: () => isManualPermissionMode(configStore.current()),
	});
	...
	const serviceLifecycle = new PermissionServiceLifecycle(permissionsService, subagentDetection, pi.events, [
	...
	const lifecycle = new SessionLifecycleHandler(session, resolver, serviceLifecycle, logger, audit);
	...
	const gateRunner = new GateRunner(resolver, sessionRules, authorizerSelection, reporter);
	const toolCallGatePipeline = new ToolCallGatePipeline(resolver, session, formatterRegistry, accessExtractorRegistry);
	...
	pi.on("resources_discover", (event, ctx) => lifecycle.handleResourcesDiscover(event, ctx));
	pi.on("session_shutdown", () => lifecycle.handleSessionShutdown());
	pi.on("before_agent_start", (event, ctx) => agentPrep.handle(event, ctx));
	pi.on("input", (event, ctx) => gates.handleInput(event, ctx));
	pi.on(
		"tool_call",
		createFailClosedToolCall((event, ctx) => gates.handleToolCall(event, ctx), reporter, audit, logger),
	);
}
