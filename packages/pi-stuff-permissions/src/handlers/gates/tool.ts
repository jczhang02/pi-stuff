...
import { classifyToolKind, type ShellInvocation } from "#src/access-intent/tool-kind";
...
import { accessFactsFromPath, accessFactsFromValue, deriveDecisionValue } from "./helpers";
 ...
 */
function deriveSuggestionValue(toolName: string, check: PermissionCheckResult, accessPath?: AccessPath): string {
...
export function describeToolGate(
	...
	const suggestion = suggestSessionPattern(gateSurface, deriveSuggestionValue(gateSurface, check, accessPath));
