	...
	private async runDescriptor(
		...
		if (check.source === "session") {
			...
			this.reporter.emitDecision(
				buildDecisionEvent(descriptor.decision, check, agentName, "allow", "session_approved"),
			);
			return { action: "allow" };
		}
		...
		this.reporter.emitDecision(
			buildDecisionEvent(
				...
				deriveResolution(check.state, gateResult.action, hasSessionApproval, confirmationUnavailable, autoApproved),
			),
		);
