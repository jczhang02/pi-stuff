...
import { resolveShellInvocation, type ShellInvocation } from "#src/access-intent/tool-kind";
import type { ShellToolsConfig } from "#src/config-schema";
import { classifyDestructiveCommand, findGitWorktreeRoot } from "#src/destructive-command";
...
import { describeDestructiveCommandGate } from "./destructive-command";
...
export class ToolCallGatePipeline {
	...
	async evaluate(tcc: ToolCallContext, runner: GateRunner): Promise<GateOutcome> {
		...
		const shell = resolveShellInvocation(tcc.toolName, tcc.input, this.inputs.getShellToolAliases());
		...
		if (shell && bashProgram) {
			const tripwireCwd = shell.workdir
				? (normalizer.forBashToken(shell.workdir).value() ?? shell.workdir)
				: tcc.cwd;
			const tripwireNormalizer =
				tripwireCwd === tcc.cwd ? normalizer : new PathNormalizer(normalizer.flavor, tripwireCwd);
			const tripwire = classifyDestructiveCommand(bashProgram, tripwireNormalizer, {
				cwd: tripwireCwd,
				gitWorktreeRoot: findGitWorktreeRoot(tripwireCwd, tripwireNormalizer),
			});
			if (tripwire.action !== "allow") {
				const outcome = await runner.run(
					describeDestructiveCommandGate(tcc, tripwire),
					tcc.agentName,
					tcc.toolCallId,
				);
		...
		const gateProducers: Array<() => GateResult | Promise<GateResult>> = [
			() => describeSkillReadGate(tcc, normalizer, () => this.inputs.getActiveSkillEntries()),
			() => describePathGate(tcc, this.resolver, normalizer, this.customExtractors),
			() => describeExternalDirectoryGate(tcc, infraDirs, this.resolver, normalizer, this.customExtractors),
			() => describeBashExternalDirectoryGate(tcc, bashProgram, this.resolver),
			() => describeBashPathGate(tcc, bashProgram, this.resolver),
			() => {
				const { toolCheck, accessPath } = this.resolvePerToolCheck(tcc, shell, bashProgram, normalizer);
				const toolDescriptor = describeToolGate(tcc, toolCheck, formatter, accessPath, shell);
