...
import { OWNER_ONLY_FILE_MODE, restrictExistingPathToOwner } from "./log-file-permissions";
...
export function createPermissionSystemLogger(options: PermissionSystemLoggerOptions): PermissionSystemLogger {
	...
	const writeLine = (
		...
		try {
			...
			if (!line) {
				return `Failed to write Pi Stuff permissions ${stream} log '${path}': event could not be serialized.`;
			}
			...
			return `Failed to write Pi Stuff permissions ${stream} log '${path}': ${message}`;
		}
	};
