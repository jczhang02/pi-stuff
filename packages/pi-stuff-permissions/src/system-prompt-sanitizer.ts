...
function narrowAvailableToolsSection(
	...
	const hasToolBullet = filteredBody.some((line) => extractToolBulletName(line) !== null);
	if (!hasToolBullet) {
...
export function sanitizeAvailableToolsSection(
	...
	const narrowedToolsSection = narrowAvailableToolsSection(normalizedLines, allowedTools);
	const sanitizedGuidelines = sanitizeGuidelinesSection(narrowedToolsSection.lines, allowedTools);
