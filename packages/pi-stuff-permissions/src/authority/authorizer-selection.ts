...
import { type Authorizer, type AuthorizerSelectionDeps, selectAuthorizer, type TerminalAuthorizer } from "./authorizer";
 ...
 */
export class AuthorizerSelection implements AskEscalator, AuthorizerSelectionLifecycle {
	...
	escalate(details: PromptPermissionDetails): Promise<PermissionPromptDecision> {
		...
		const chain = details.exactCallOnly
			? this.terminal
