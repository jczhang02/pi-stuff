...
export interface RequestPermissionOptions {
	/** Show only one-call approval and deny; never create a remembered grant. */
	exactCallOnly?: boolean;
	/** Structured, untrusted evidence for the bounded exact-call review viewport. */
	exactCallEvidence?: {
		requester: string;
		command: string;
		reason: string;
...
export async function requestPermissionDecisionFromUi(
	...
	if (options?.exactCallOnly) {
		const selected = await ui.select(`${title}\n${message}`, ["Allow this exact call once", DENY_OPTION]);
		return selected === "Allow this exact call once"
