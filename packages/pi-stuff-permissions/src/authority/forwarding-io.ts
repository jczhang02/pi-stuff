...
import { OWNER_ONLY_DIRECTORY_MODE, OWNER_ONLY_FILE_MODE } from "#src/log-file-permissions";
 ...
 */
function asForwardedSessionApproval(value: unknown): ForwardedSessionApproval | undefined {
...
function asTripwireEvidence(value: unknown): NonNullable<ForwardedPermissionRequest["tripwire"]> | undefined {
 ...
 */
function asForwardedAccessIntent(value: unknown): ForwardedAccessIntent | undefined {
	if (typeof value !== "object" || value === null) {
		return undefined;
	}
	const candidate = value as {
	...
	if (
		...
		!(candidate.boundaryValue === null || typeof candidate.boundaryValue === "string") ||
...
export function readForwardedPermissionRequest(
	...
	try {
		...
		const tripwire = asTripwireEvidence(parsed.tripwire);
		return {
			id: parsed.id,
			...
			...(parsed.exactCallOnly === true ? { exactCallOnly: true } : {}),
			...(tripwire ? { tripwire } : {}),
