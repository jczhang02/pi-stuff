...
export interface PromptModelConfig {
	...
	/** Restrict the dialog to allow-once and deny. */
	exactCallOnly: boolean;
...
export function initialPromptState(_config: PromptModelConfig): PromptViewState {
	return {
		step: "decision",
		highlightedKey: "y",
		armedKey: undefined,
 ...
 */
export function reducePrompt(config: PromptModelConfig, state: PromptViewState, event: PromptEvent): PromptOutcome {
	switch (state.step) {
		case "decision":
...
function reduceDecisionStep(config: PromptModelConfig, state: PromptViewState, event: PromptEvent): PromptOutcome {
	switch (event.type) {
		case "nav":
			return render({
				...state,
				highlightedKey: shiftKey(state.highlightedKey, event.direction, config),
				armedKey: undefined,
				hint: "",
			});
		case "hotkey":
			if (!optionOrder(config).includes(event.key)) return render(state);
...
function pressHotkey(config: PromptModelConfig, state: PromptViewState, key: PromptKey): PromptOutcome {
...
function commit(config: PromptModelConfig, state: PromptViewState, key: PromptKey): PromptOutcome {
	switch (key) {
		case "y":
			return {
				kind: "decision",
...
function reduceReasonStep(state: PromptViewState, event: PromptEvent): PromptOutcome {
...
function reduceScopeStep(state: PromptViewState, event: PromptEvent): PromptOutcome {
	switch (event.type) {
		case "nav":
...
function shiftKey(current: PromptKey, direction: "up" | "down", config?: PromptModelConfig): PromptKey {
	const order = config ? optionOrder(config) : OPTION_ORDER;
...
function optionOrder(config: PromptModelConfig): readonly PromptKey[] {
	return config.exactCallOnly ? ["y", "n"] : OPTION_ORDER;
}
