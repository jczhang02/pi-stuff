 ...
 * `import("@jczhang02/pi-stuff-permissions/service")` gets a fresh module copy, but
 ...
 * log — `review` for the optional durable audit entry (one per handled ask),
 * `debug` for verbose or short-circuit detail gated behind the
 * `debugLog` toggle. The session's own logger is passed straight through, so a
 * link's entries land in the same `pi-stuff-permissions-permission-review.jsonl`
...
const SERVICE_KEY = Symbol.for("@jczhang02/pi-stuff-permissions:service");
 ...
 * Pi Stuff permissions extension has not loaded (or has been unloaded).
 */
