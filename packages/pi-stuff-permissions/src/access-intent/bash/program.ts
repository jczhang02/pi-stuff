...
import { BashPathResolver, type BashPathRuleCandidate } from "#src/access-intent/bash/bash-path-resolver";
import { type BashCommand, collectCommands } from "#src/access-intent/bash/command-enumeration";
import { type BashInvocation, collectInvocations } from "#src/access-intent/bash/invocation-enumeration";
import { getParser } from "#src/access-intent/bash/parser";
import type { PathNormalizer } from "#src/path-normalizer";

export type { BashCommand, BashInvocation, BashPathRuleCandidate };
...
export class BashProgram {
	private constructor(
		...
		private readonly invocationUnits: readonly BashInvocation[],
		...
		private readonly syntaxInvalid: boolean,
	) {}
	...
	static async parse(
		...
		if (!tree) return new BashProgram(command, [], [], [], [], true);

		try {
			const { externalPaths, ruleCandidates } = new BashPathResolver(normalizer, options?.workdir).resolve(
				tree.rootNode,
			);
			return new BashProgram(
				command,
				collectCommands(tree.rootNode),
				collectInvocations(tree.rootNode, normalizer, options?.workdir),
				externalPaths,
				ruleCandidates,
				tree.rootNode.hasError === true,
			);
		} finally {
			tree.delete();
		}
	...
	/** Command occurrences with statically tracked effective directories. */
	invocations(): BashInvocation[] {
		return [...this.invocationUnits];
	}

	/** Whether tree-sitter recovered from invalid or incomplete shell syntax. */
	hasSyntaxError(): boolean {
		return this.syntaxInvalid;
	}
