...
export function formatAskPrompt(
	...
	if (classifyToolKind(result.toolName) === "bash") {
		...
		const qualifier = matchQualifier(result.matchedPattern, result.commandContext);
		const qualifierInfo = qualifier ? ` ${qualifier}` : "";
		const fullCommand = getNonEmptyString(toRecord(input).command);
		const fullCommandInfo = fullCommand && fullCommand !== subCommand ? ` (full command: '${fullCommand}')` : "";
	...
	if (isMcpCheck(result) && result.target) {
		...
		const mcpPreview = formatter ? formatter.formatToolInputForPrompt("mcp", input) : "";
	...
	const patternInfo = result.matchedPattern ? ` (matched '${result.matchedPattern}')` : "";
	const inputPreview = formatter ? formatter.formatToolInputForPrompt(result.toolName, input) : "";
