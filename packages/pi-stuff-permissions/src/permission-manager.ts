...
import { getGlobalConfigPath, getProjectAgentsDir, getProjectConfigPath } from "./config-paths";
...
import { evaluate, evaluateAnyValue, evaluateFirst, floorAllowsToAsk, rewriteAsksToYolo } from "./rule";
...
const DEFAULT_UNIVERSAL_FALLBACK: PermissionState = "allow";
...
type ResolvedPermissions = {
	...
	/** User-owned global denies, applied after every lower-trust and session rule. */
	globalDenyFloor: Ruleset;
	/**
	 * Scopes whose config file failed to load or validate. When
...
export interface PermissionManagerOptions extends PolicyLoaderOptions {
	...
	/** Live user-owned mode; manual floors only the builtin fallback to ask. */
...
export class PermissionManager implements ScopedPermissionManager {
	...
	private readonly isManualModeEnabled: () => boolean;
	...
	constructor(options: PermissionManagerOptions = {}) {
		...
		this.isManualModeEnabled = options.isManualModeEnabled ?? YOLO_DISABLED;
		this.loader =
	...
	private resolvePermissions(agentName?: string): ResolvedPermissions {
		...
		const globalDenyFloor: Ruleset = normalizeFlatConfig(globalConfig.permission ?? {})
			.filter((rule) => rule.action === "deny")
			.map((rule) => ({ ...rule, layer: "config", origin: "global" }));
		...
		// Fail closed when any scope's config is invalid: floor every
	 ...
	 * Used by the `/permissions` command to display effective rules
	...
	getComposedConfigRules(agentName?: string): Ruleset {
		const { composedRules, globalDenyFloor } = this.resolvePermissions(agentName);
		return [...composedRules.filter((r) => r.layer === "config"), ...globalDenyFloor];
	}
	...
	getToolPermission(toolName: string, agentName?: string): PermissionState {
		...
		return evaluate(
			toolName.trim(),
			"*",
			[...this.applyPermissionMode(composedRules), ...globalDenyFloor],
			this.flavor,
		).action;
	}
	 ...
	 */
	check(intent: ResolvedAccessIntent, sessionRules?: Ruleset): PermissionCheckResult {
		const { composedRules, globalDenyFloor } = this.resolvePermissions(intent.agentName);
		const modeRules = this.applyPermissionMode(composedRules);
		const composedWithSession: Ruleset = sessionRules?.length
			? [...modeRules, ...sessionRules, ...globalDenyFloor]
			: [...modeRules, ...globalDenyFloor];
		...
		const fullRules: Ruleset = this.isYoloEnabled() ? rewriteAsksToYolo(composedWithSession) : composedWithSession;
		...
		return buildCheckResult(surface, values, resultExtras, toolName, intent.surface, fullRules, this.flavor);
	}

	private applyPermissionMode(rules: Ruleset): Ruleset {
		if (!this.isManualModeEnabled()) return rules;
		return rules.map((rule) =>
			rule.layer === "default" && rule.origin === "builtin" && rule.action === "allow"
				? { ...rule, action: "ask" }
				: rule,
		);
	}
 ...
 */
function derivePolicyLoaderOptions(agentDir: string, cwd: string | undefined | null): PolicyLoaderOptions {
	return {
