...
import type { ShellToolsConfig, UnifiedPermissionConfig } from "./config-loader";
import { OWNER_ONLY_DIRECTORY_MODE, restrictExistingPathToOwner } from "./log-file-permissions";

export const EXTENSION_ID = "pi-stuff-permissions";

export interface PermissionSystemExtensionConfig {
	permissionMode: "unrestricted" | "manual";
	debugLog: boolean;
	permissionReviewLog: boolean;
	yoloMode: boolean;
	/** Require a confirming second press of a decision hotkey in the TUI dialog. Defaults to false. */
...
export function normalizePermissionSystemConfig(raw: UnifiedPermissionConfig): PermissionSystemExtensionConfig {
	const result: PermissionSystemExtensionConfig = {
		permissionMode: raw.permissionMode === "manual" ? "manual" : "unrestricted",
		debugLog: raw.debugLog === true,
		permissionReviewLog: raw.permissionReviewLog === true,
...
export function isManualPermissionMode(config: PermissionSystemExtensionConfig): boolean {
...
export function ensurePermissionSystemLogsDirectory(logsDir: string): string | undefined {
	try {
		...
		return `Failed to create Pi Stuff permissions log directory '${logsDir}': ${message}`;
	}
}
