...
## [2.19.0-pi-stuff.7] - 2026-08-04

### Fixed
- Removed the redundant Zod peer declaration because Zod is already an exact
  bundled runtime dependency. This prevents Bun 1.3.14 from emitting a
  duplicate nested lockfile key when the owned fork is embedded in Pi Stuff.

## [2.19.0-pi-stuff.6] - 2026-08-04

### Fixed
- Explicitly terminates established Streamable HTTP sessions before closing
  their SDK clients, including the transport capability probe, so remote
  servers do not retain abandoned sessions.

## [2.19.0-pi-stuff.5] - 2026-08-04

### Changed
- Removed regex MCP tool discovery and its heavyweight native and Java safety
  backends from the bounded Pi Stuff distribution. Literal ranked discovery
  remains the supported search surface.

## [2.19.0-pi-stuff.4] - 2026-08-04

### Changed
- Added an embedding option that disables MCP sampling and elicitation
  capability advertisement, preventing protocol callbacks from opening native
  prompts in Pi Stuff.

## [2.19.0-pi-stuff.3] - 2026-08-04

### Fixed
- Bundled the complete runtime dependency graph so the owned release asset is
  self-contained when embedded in another Pi package.

## [2.19.0-pi-stuff.2] - 2026-08-04

### Changed
- Added a declaration-only root type entry so strict TypeScript hosts do not
  re-check the fork's runtime source under unrelated compiler policies.

## [2.19.0-pi-stuff.1] - 2026-08-04

### Changed
- Added a proxy-only embedding mode that excludes direct and script Tool
  surfaces even when user configuration requests them.

## [2.19.0-pi-stuff.0] - 2026-08-04

### Changed
- Added an embedding option that defers every configured connection until an
  explicit MCP operation.
- Required an explicit opt-in before MCP Apps may open browser or native UI.
- Qualified against Pi 0.83, pinned runtime dependencies, and removed bundled
  Skills and media from the owned Pi Stuff distribution.
