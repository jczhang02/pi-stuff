...
## [0.18.0-pi-stuff.4] - 2026-08-04

### Added
- Added validated process-local SSRF defaults for embedding hosts. Explicit
  `web-search.json` fields still take precedence, including an empty
  `ssrf.allowRanges` array, and the embedding API never writes user settings.

## [0.18.0-pi-stuff.3] - 2026-08-04

### Added
- Added an embedding factory that can keep GitHub access API/HTTP-only and
  bypass YouTube-specific Chrome, Gemini, and video extraction without changing
  the default upstream behavior.

## [0.18.0-pi-stuff.2] - 2026-08-04
...
## [0.18.0-pi-stuff.1] - 2026-08-04

### Changed
- Added a declaration-only type entry so strict TypeScript hosts do not
  re-check the fork's runtime source under unrelated compiler policies.

## [0.18.0-pi-stuff.0] - 2026-08-04

### Changed
- Renamed the owned distribution and pinned its dependency graph for Pi Stuff.
- Removed non-runtime media from the published archive.
- Kept browser curation, commands, shortcuts, and Tool presentation under the
  Pi Stuff adapter rather than enabling them directly in the Suite.
