...
/** Stable launch identity shared by the public tool and session-wide governor. */
export function deriveLaunchRunId(toolCallId: string): string {
	return createHash("sha256").update(toolCallId).digest("hex").slice(0, 12);
}
...
function prepareLaunch(
	...
	const runId = deriveLaunchRunId(id);
...
function launchBackground(
	...
	if (data.mode === "parallel") {
		...
		return engines.backgroundParallel(data.runId, {
			...common,
			agents: data.agents,
			tasks,
...
function buildForegroundConfig(
	...
	ctx: ExtensionContext,
	deps: ExecutorDeps,
): BackgroundRunnerConfig | AgentToolResult<Details> {
	const common = commonBuild(data, ctx, deps);
	...
	const asyncDir = path.join(TEMP_ROOT_DIR, "foreground-runs", data.runId);
...
function emitNestedLifecycle(data: PreparedLaunch, result?: AgentToolResult<Details>): void {
	...
	try {
		writeNestedEvent(data.inheritedNestedRoute, {
			type: result ? "subagent.nested.completed" : "subagent.nested.started",
			ts: now,
			parentRunId: data.nestedParentAddress.parentRunId,
			parentStepIndex: data.nestedParentAddress.parentStepIndex,
			child: {
				...
				...(result?.details.results.length
					? {
							steps: result.details.results.map((child) => ({
								agent: child.agent,
...
async function launchForeground(
	...
	const config = buildForegroundConfig(data, ctx, deps);
	...
	rememberForegroundResult(deps.state, data, result, control.startedAt);
	emitNestedLifecycle(data, result);
	for (const [index, child] of result.details.results.entries()) {
		deps.pi.events.emit(SUBAGENT_FOREGROUND_COMPLETE_EVENT, {
			...
			exitCode: child.exitCode,
			state: child.stopped ? "stopped" : child.interrupted ? "paused" : child.exitCode === 0 ? "complete" : "failed",
			timestamp: Date.now(),
			cwd: data.effectiveCwd,
...
function foregroundResumeTarget(params: SubagentParamsLike, state: SubagentState) {
	const requested = params.id;
	if (!requested) return undefined;
	const candidates = [...(state.foregroundRuns?.values() ?? [])].filter(
		(run) => run.sessionId === state.currentSessionId && run.runId.startsWith(requested),
	);
	const run =
		candidates.find((candidate) => candidate.runId === requested) ??
		(candidates.length === 1 ? candidates[0] : undefined);
	if (!run) return undefined;
	if (candidates.length > 1 && !candidates.some((candidate) => candidate.runId === requested)) {
		throw new Error(
			`Agent id '${requested}' is ambiguous: ${candidates.map((candidate) => candidate.runId).join(", ")}.`,
		);
	}
...
async function resumeRun(input: {
	...
	try {
		target =
			foregroundResumeTarget(input.params, input.deps.state) ??
			resolveAsyncResumeTarget(
				{ id: input.params.id, index: input.params.index },
				{ kill: input.deps.kill },
				{ requireSessionFile: true, sessionId: input.deps.state.currentSessionId ?? undefined },
			);
	} catch (error) {
		return errorResult("management", error instanceof Error ? error.message : String(error));
	}
