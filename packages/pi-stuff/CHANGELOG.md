...
- Patch Magic Context 0.40.0's packaged tokenizer resolver to search its own Bun module ancestry, restoring exact token
  counts and preventing raw `[magic-context]` fallback warnings from flashing in the Pi editor. Add real-Host PTY
  regression coverage and remove the patch once an equivalent official release passes certification.
