...
outcome. Each nested call keeps the exact renderer, Tool Activity metadata, streaming state, failure state, expansion
behavior, and media behavior of the original Tool. A missing historical Tool definition or a failing renderer receives
a generic Tool row at the same source position. The outer result stores the nested calls in normal Pi Session JSONL
details, so reload and resume
