...
- Call only listed or searched methods, e.g. codemode.search("view image"); inspect unfamiliar methods with codemode.describe("tools.name"). If exact input fields are not visible, including after context compaction, describe the method before calling and never guess input field names. Do not guess Tool names.
