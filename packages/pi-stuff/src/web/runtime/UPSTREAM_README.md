...
### Owned-fork embedding

The default extension preserves upstream behavior. Hosts embedding this owned
fork can explicitly keep GitHub access API/HTTP-only and route YouTube URLs
through ordinary HTTP extraction:
...
An embedding host may also set validated, process-local SSRF defaults after a
user-triggered network check. Explicit user configuration always wins, and an
explicit empty range list disables the host default:

```typescript
import { configureRuntimeSsrfDefaults } from "@jczhang02/pi-web-access";

configureRuntimeSsrfDefaults({ allowRanges: ["198.18.0.0/15"] });
```

This API does not read or write settings. Embedding hosts are responsible for
establishing that an allowed range belongs to their active proxy before using
it.
...
Works immediately with no API keys — Exa MCP provides zero-config search. If Pi has Codex auth from `/login`, OpenAI search can also work without a separate key. For more providers or direct API access, add keys to the `web` namespace in `<Pi agent dir>/pi-stuff.json`:
...
Persists to the `web` namespace in `<Pi agent dir>/pi-stuff.json` and takes effect on the next `web_search` call. When disabled, `web_search` returns raw results without opening the curator window.
...
Both shortcuts are configurable via the `web` namespace in `<Pi agent dir>/pi-stuff.json`:
