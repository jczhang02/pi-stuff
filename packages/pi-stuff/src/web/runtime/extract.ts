...
export interface ExtractOptions {
	timeoutMs?: number;
	forceClone?: boolean;
	/** Internal embedding policy. Defaults to true for upstream-compatible behavior. */
	allowGitHubClone?: boolean;
	/** Internal embedding policy. Defaults to true for upstream-compatible behavior. */
	allowYouTubeSpecialization?: boolean;
	prompt?: string;
	timestamp?: string;
	frames?: number;
	model?: string;
...
async function extractLocalFrames(
	...
	const results = await Promise.all(timestamps.map(async (t) => {
		const frame = await fetchLimit(() => extractVideoFrame(filePath, t, signal));
...
export async function extractContent(
	...
	try {
		const ghResult = await extractGitHub(url, signal, options?.forceClone, options?.allowGitHubClone);
	...
	if (options?.allowYouTubeSpecialization !== false) {
		try {
