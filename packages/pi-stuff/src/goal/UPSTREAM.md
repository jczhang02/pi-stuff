...
Pi Stuff does not consume the upstream Package at runtime. The pinned source snapshot was absorbed into this module and
is maintained under the original MIT license without an independent Package lifecycle.
...
- Publishes observation-only Goal state through the internal Conversation UI contract; Goal does not own the Statusline.
