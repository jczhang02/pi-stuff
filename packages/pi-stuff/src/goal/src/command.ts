...
export function completeGoalArguments(
	...
	const completions = features.experimentalGoals
		? [...GOAL_ARGUMENT_COMPLETIONS.slice(0, -1), ...QUEUE_ARGUMENT_COMPLETIONS, TOKEN_BUDGET_COMPLETION]
	...
	if (objectiveOption) {
		...
		const optionPrefix = features.experimentalGoals ? (objectiveOption[2] ?? "") : (objectiveOption[1] ?? "");
...
export function parseCommand(args: string, features: GoalCommandFeatures = {}): CommandResult | string {
...
function parseObjective(kind: "start" | "edit" | "add" | "prioritize", tokens: string[]): CommandResult | string {
