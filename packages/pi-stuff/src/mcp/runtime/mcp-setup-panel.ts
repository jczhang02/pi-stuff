...
type PendingWrite = { readonly kind: "action"; readonly action: Action } | { readonly kind: "imports" };
...
export class McpSetupPanel {
	...
	private renderConfirmation(innerW: number): McpDialogRows {
		...
		const preview = this.safePreview(
			() =>
				pending.kind === "imports"
					...
					: this.getActionPreview(action, this.previewWidth(innerW)),
			this.previewWidth(innerW),
		);
		...
		const selected = lines.at(this.confirmCursor === 0 ? -3 : -2);
	...
	private renderActions(innerW: number): McpDialogRows {
		...
		if (innerW >= COMPACT_WIDTH) {
			...
			const preview = this.safePreview(
				() => this.getActionPreview(this.getSelectedAction() ?? undefined, this.previewWidth(innerW)),
				this.previewWidth(innerW),
			);
