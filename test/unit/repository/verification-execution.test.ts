...
async function fixture(): Promise<{ root: string; head: string; file: string }> {
	...
	await mkdir(join(root, "test/unit/example"), { recursive: true });
	await writeFile(
		join(root, "test/unit/example/example.test.ts"),
	...
	return { root, head, file: "test/unit/example/example.test.ts" };
}
...
test("verification plan rejects stale, malformed, and contradictory execution scope", async () => {
	...
	try {
		...
		for (const bad of [
			...
			{ ...valid, files: [] },
			{ ...valid, files: ["test/unit/example/unknown.test.ts"] },
			{ ...valid, files: ["test/unit/example/example-live.test.ts"] },
			{ ...valid, files: ["../escape"] },
