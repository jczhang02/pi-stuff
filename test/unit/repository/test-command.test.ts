...
test("test preview selects files without executing them and rejects empty selections", async () => {
	...
	try {
		...
		await mkdir(join(cwd, "test/system-integration/subagents"), { recursive: true });
		await writeFile(
			join(cwd, "test/system-integration/subagents/process-controls-recovery.test.ts"),
			'throw new Error("scenario executed");',
		);
...
test("the runner passes the installed Pi instead of a development SDK CLI", async () => {
	...
	try {
		...
		await writeFile(
			join(cwd, "test/unit/example/pure.test.ts"),
