...
async function fixture(row: string): Promise<string> {
	...
	await write(root, "test/demo.test.ts", "export {};\n");
	await write(
		root,
...
test("accepts a structurally complete pending contract", async () => {
	const root = await fixture(
		"| `demo.surface` | `demo` | [README](../packages/pi-stuff/src/demo/README.md) | Host command | normal: opens; failure: reports errors; recovery: retries | `real-host` | [test](../test/demo.test.ts) | `pending` |",
	);
...
test("requires a report link for a completed status", async () => {
	const root = await fixture(
		"| `demo.surface` | `demo` | [README](../packages/pi-stuff/src/demo/README.md) | Host command | normal: opens | `real-host` | [test](../test/demo.test.ts) | `pass` |",
	);
...
test("requires a date in a completed status report path", async () => {
	const root = await fixture(
		"| `demo.surface` | `demo` | [README](../packages/pi-stuff/src/demo/README.md) | Host command | normal: opens | `real-host` | [test](../test/demo.test.ts) | [pass](reports/acceptance.md) |",
	);
	await write(
		root,
...
test("matches a completed status to its report matrix", async () => {
	const root = await fixture(
		"| `demo.surface` | `demo` | [README](../packages/pi-stuff/src/demo/README.md) | Host command | normal: opens | `real-host` | [test](../test/demo.test.ts) | [blocked](reports/acceptance-2026-09-02.md) |",
	);
	await write(
		root,
...
test("rejects any non-repository evidence link beside valid evidence", async () => {
	const root = await fixture(
		"| `demo.surface` | `demo` | [README](../packages/pi-stuff/src/demo/README.md) | Host command | normal: opens | `real-host` | [test](../test/demo.test.ts); [external](https://example.com) | `pending` |",
	);
	expect(await auditCapabilityContractCatalog(root)).toContainEqual({
		path: "docs/capability-contract-catalog.md",
		rule: "evidence:demo.surface",
