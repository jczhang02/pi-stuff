...
### 3. Build the shareable terminal demo
...
Write it for a non-developer. Every label is in **domain language**, not code: actions and state read like the business, not the reducer. Explain in plain words what's happening.

Lay it out with a clean hierarchy, top to bottom:

1. **Title and one-line explanation** of what this demo lets you explore (the question from step 1).
2. **Current state**: the full relevant state, rendered as a readable panel (labelled fields, not a raw JSON dump), re-rendered after every action so the change is visible. Where it helps a non-developer follow, call out what just changed.
3. **Free-play actions**: one terminal control per action, always available, so anyone can poke at the model in any order. Each activation dispatches its action and re-renders the state.
4. **Guided walkthroughs**: a set of **scenarios**, one per tab in the terminal view. Each tab holds a short plain-language description of the scenario (the situation it sets up and what to watch for) and underneath it, the ordered **actions to take** for that scenario. Each step is an interactive control: activating it performs that action and moves to the next step. Starting a walkthrough resets to a known initial state so the scenario runs the same way every time.
...
Keep it beautiful but restrained: readable terminal text, spacing suited to the available width, and the project's theme. No animations, no gimmicks: nothing that competes with the state and the controls.
