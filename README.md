...
[Subagents](docs/subagents.md) adds parallel and dependent delegation, targeted messages, stop/resume, completed-child follow-up and retained request history. FleetView and the bottom inspector keep the main conversation visible. See the [acceptance record](docs/subagents-redevelopment-acceptance.md) for the pinned SDK and compiled-host checks.
...
The tested target is Linux Bun-compiled Pi `0.85.1`. The stack is TypeScript, Bun `1.4.0` and Effect v4 (`4.0.0-rc.112`); pure algorithms remain ordinary functions. Other host coverage is recorded per capability. Beyond Web access, RTK integration, subagents and themes, the capabilities below describe product direction, not implemented features.
...
- [src/subagent/](src/subagent/) owns delegation, native child sessions, worktrees, retained records and inspection UI.
...
| [Subagents](docs/subagents.md)                                 | Delegation, inspection, intervention, continuation and code handoff |
