...
- **Semantic Session names** — the first settled direct-user exchange receives a bounded model-generated name;
  `/autoname` refreshes it explicitly, and automatic naming never takes over Child Agent Sessions.
...
| `/autoname` | Regenerate the current Session name from bounded recent conversation context |
...
| `session-naming` | Bounded semantic Session names after settled direct-user work, branch-local ownership state, and `/autoname` |
