...
> [!IMPORTANT]
> The current Package version is `0.2.2`. Registry publication remains a separate, explicitly approved maintainer action.

See the [0.2.2 release notes](docs/releases/0.2.2.md) for the current scope, compatibility boundary, and acceptance gates.
...
| --- | --- | --- |
| `@jczhang02/pi-stuff` | Aggregate entry point for the full Suite | `0.2.2` |
| `@jczhang02/pi-stuff-ui` | Responsive Statusline, input presentation, and shared non-floating Command Dialog coordinator | `0.2.2` |
| `@jczhang02/pi-stuff-tools` | Compact built-in and Suite-owned Tool presentation with focused details | `0.1.3` |
| `@jczhang02/pi-stuff-code-mode` | One-schema local JavaScript composition for active Suite Tools with unchanged Tool UI | `0.1.0` |
| `@jczhang02/pi-stuff-context` | Lazy Magic Context continuity with native Pi fail-open behavior | `0.1.3` |
| `@jczhang02/pi-stuff-rtk` | Certified fail-open RTK rewriting and model-only Tool output projection | `0.2.2` |
| `@jczhang02/pi-stuff-codex` | Codex Fast and usage controls plus selected patch and image Tools | `0.1.2` |
| `@jczhang02/pi-stuff-goal` | Persistent, evidence-gated continuation for one current-session objective | `0.2.2` |
| `@jczhang02/pi-stuff-web` | Bounded public Web search, page/PDF reading, and continuation retrieval | `0.2.2` |
| `@jczhang02/pi-stuff-mcp` | Lazy proxy-only MCP gateway with stdio/HTTP transports and non-floating status UI | `0.2.2` |
| `@jczhang02/pi-stuff-agents` | Current-session foreground and background Agents | `0.2.2` |
| `@jczhang02/pi-stuff-todo` | Recoverable per-session task tracking with a compact editor widget | `0.1.3` |
| `@jczhang02/pi-stuff-btw` | One-shot side questions isolated from the main transcript | `0.1.3` |
