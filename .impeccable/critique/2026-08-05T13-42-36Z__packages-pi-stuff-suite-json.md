...
Tool 标签、未激活 Todo、cwd/branch 和 Fleetview 名称在部分暗色证据中太淡。尤其 Todo 的“普通待办”和“被阻塞待办”都使用 `□`，主要差异落在明暗上；用户可能直接看不出阻塞状态。

建议：`dim` 只用于分隔符、快捷键、过期/已完成元数据；身份和状态用 `text`，必要的次级上下文用 `muted`；blocked Todo 增加文字或不同形状。随后保留一套同状态的暗色、亮色和无 ANSI 对照证据。
