...
- When delivering a fully implemented and verified spec, report added, deleted, and net changed lines for the entire spec against its pre-implementation baseline, identifying the compared commits. Report production code and tests separately; exclude documentation, lockfiles, generated files, and unrelated changes. A spec is an agreed scope with acceptance criteria recorded in a spec document or issue. This report is required at implementation delivery, without waiting for merge, but not for interim progress or ordinary non-spec work.
...
- Terminal E2E/interactive TUI automation: `.agents/skills/terminal-control/SKILL.md` and `docs/quality-assurance.md#terminal-e2e`. Use Terminal Control by default. CUA is a display-only route: require a dedicated display distinct from the host, private session D-Bus and isolated runtime with no host-foreground switch, verify host focus, pointer and clipboard, and fail closed when isolation is unavailable. Test organization, environment selection or verification/CI policy: `docs/quality-assurance.md`.
