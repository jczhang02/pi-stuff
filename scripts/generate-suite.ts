...
function parseCapabilities(registry: SuiteRegistry): CapabilityModule[] {
	...
	if (!Array.isArray(registry.capabilities) || !registry.capabilities.every((value) => isRuntimeString(value))) {
...
function parseTools(registry: SuiteRegistry, field: "tools" | "optionalTools" | "deferredTools"): string[] {
	...
	if (!Array.isArray(value) || !value.every((entry) => isRuntimeString(entry) && entry.length > 0)) {
