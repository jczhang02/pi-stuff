...
export default function (pi: ExtensionAPI) {
  pi.registerCommand('host-session', {
    description: 'Offline acceptance fixture: create or restore a session',
    handler: async (target, ctx) => {
      if (target.trim() === 'new') {
        await ctx.newSession({
          withSession: async fresh => {
            fresh.ui.notify('HOST_SESSION_NEW', 'info');
          },
        });
      } else {
        await ctx.switchSession(target.trim());
      }
    },
  });
