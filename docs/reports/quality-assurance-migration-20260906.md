...
The source-size comparison uses the JS/TS path set from the `35225c57` to working-tree git diff, including new untracked source paths and excluding artifacts: 810 physical lines became 1,760 (+950). This is a changed-path comparison, not whole-repository size or a speedup claim.

Local focused evidence completed: 9 planning tests, 3 execution/contract tests, 17 CI aggregate tests, and 3 runner tests. The complete static checks and Standards/Spec review passed, followed by two consecutive clean independent Thermo-Nuclear rounds. The first full offline run executed all 333 files and 2,458 native cases with zero skips in 1,211.4 seconds, plus 7.3 seconds of Goal setup. It failed three files: UI Host and RTK metadata tests retained old directory roots, and one embedded-status PTY color assertion failed while sampling separate snapshots. Both paths were corrected and passed focused checks; the status test now synchronizes and asserts one ANSI frame without changing its color requirements. Twelve real PTY mode/theme repetitions passed after that change. Final full offline and hosted verification remain pending. The first hosted attempt failed before execution on the deprecated cache backend; the supported pinned cache release allows Plan and Checks to pass. Baseline main CI took 89 seconds but skipped Acceptance, so it is not equivalent full-suite evidence and cannot support an overall speedup claim.
