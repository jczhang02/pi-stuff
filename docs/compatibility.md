...
unused-code analysis, generated composition, and public-release safety. `Acceptance` downloads and verifies the
certified Host release and RTK runtime before running every test file in a fresh Bun process, real TUI verification, the
Tool Activity benchmark, and package verification in a network-isolated namespace. Per-file process isolation prevents one process- or PTY-heavy
