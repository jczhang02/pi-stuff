...
| Command | Action |
| --- | --- |
| `/resume` | Open Pi's native selector with lightweight Fast Resume loaders; fall back to Pi's complete-history loader when interception is unavailable |
| `/fast-resume` | Open the native selector with lightweight loaders when `/resume` interception is disabled or unavailable |
