...
下方实验比较提前注册、SDK 查询和范围更窄的 TUI 查询. 这些工具接入候选均未获得生产批准.
...
**待批准的实现范围:** 评估用一个 TUI 查询适配器同时处理已有定义和缺失的历史 Web 展示, 不叠加 SDK 与 TUI 两层 patch. 保留执行、注册、配置校验及第三方 renderer, 仅适配所属运行实例. 分组恢复后复用原生结果失效回调. 检查私有宿主结构, 不兼容时回退原生展示; 卸载时先停用, 再有条件恢复. 不修改 Pi 安装文件或会话记录.

独立审查确认了更窄的查询范围及限定实验结果. 尚未证明全部工具、第三方归属、非法配置恢复、WebSearch/WebRead、局部鼠标展开、导出或性能. 这些检查以及现有产品历史测试失败仍待生产验收. 此提案取代先前 SDK lookup 建议, 不代表生产批准.
