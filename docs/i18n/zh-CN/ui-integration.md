...
真实宿主回归检查混合检索聚合、reload、新建会话再 resume、鼠标展开左侧对齐、记录字节不变及源文件变化后保留旧 Grep 输出. 测试在迁移前失败、迁移后在两个支持宿主通过. 该阶段 Write、Edit、Bash 和缺失的 Web 历史展示仍待迁移.

### Write 和 Edit 历史

Write 和 Edit 现通过同一 TUI 查询装饰宿主定义, 已删除替换注册及原生工厂调用. 执行和 schema 保留宿主引用, 既有预览、语法高亮及 diff 组件不变. renderer 输入在展示边界解码.

新增真实宿主回归在修改前因 reload 退回原生样式而失败. 修改后, 两个支持宿主在 reload、新建会话再 resume 后均保留 Write 预览和 Edit diff, 即使磁盘文件已被修改. 展开使用记录中的正文与 patch. 测试确认恢复不会改写会话或文件. 加上既有预览、代码设置、主题/宽度、部分参数及取消检查, 每个宿主通过11项测试、107次断言. 该阶段 Bash 和缺失的 Web 历史展示仍待接入; 这些短测试不代表长会话性能已验收.
...
该阶段缺失的 Web 历史展示及完整性能/兼容验收仍待完成.

### 缺失的 Web 展示

当实际定义和现存 TUI view 都不存在时, TUI 适配器现提供仅含 renderer 的 WebSearch、WebFetch 和 WebRead 展示. 当前注册与历史 fallback 共用同一展示代码, 不创建占位 execute、参数 schema 或可调用工具. 模型校验和 Web 注册仍在原有 session-start 回调中执行, SDK 查询不变. 现存定义及第三方 view 优先.

混合本地/Web reload 回归在本段修改前失败、修改后通过. 两个宿主各通过15项针对性测试、131次断言, 包括禁用 Web 后 reload/new/resume 的历史、保留抓取/读取正文和搜索认证错误、记录字节不变、不新增本地请求, 以及后续模型请求不包含禁用工具. 同一运行实例切换会话也保留 Web 聚合. 既有批次元信息、空结果、警告、并行调用及取消检查通过.

缺失 view 的 fallback 识别三个自有 API 名称. 保存的调用不包含已卸载扩展的 renderer 身份, 因而这些检查不能证明使用同名工具的缺失第三方历史归属. 现存第三方定义保持优先; 任意扩展注册顺序及完整兼容/性能验收仍待完成. 核心适配器现为97行, 包含注释和空行.

### 压缩与检索边界

轮次中途压缩暴露了一个展示错误: 三条检索的第一条被摘要替代后, 剩余两条仍指向已移除的组首项, 收起态完全消失. 索引现使用 Pi 的 `buildContextEntries()` 恢复, 并监听 `session_compact`, 在 Pi 重建会话展示之前更新. 不修改压缩过程或会话存储.

Pi 将最新压缩摘要放到上下文列表开头. 索引使用摘要记录已有的 `parentId` 保留实际时间位置的边界, 让压缩后直接继续的工具调用另起一组. 缺少这一步时, 恢复的两条检索会与后续一条错误合并为 `Read 3 files`.

两个失败均在修复前实际复现. 系统测试通过原生 `/compact` 使用固定摘要和合法 assistant 消息切点, 检查磁盘修改及 reload 后保留旧结果; 另用 Pi 公开 SessionManager 和实际工具消息构造会话, 恢复后检查继续调用的边界. 这些测试不触发自动上下文溢出, 也未使用真实摘要模型. 同批回归包括 tree/fork、检索失败/取消及 Web 历史.

下图为编译 Pi 0.87.0 / Bun 1.4.0 压缩后展开保留的两条检索. Terminal Control 导出100×28暗色主题画面, 使用 `JetBrainsMono Nerd Font Mono, Symbols Nerd Font Mono, LXGW WenKai Mono`, 不是 Ghostty 窗口截图.

![轮次中途压缩后保留的检索](../../assets/ui/compaction-history-dark-100.png)
