...
- 使用简洁、直接的技术语言。解释必要术语，省略空话。提交、Issue、PR 评论和代码中不使用 emoji。
- 解释复杂问题时给出具体内容：说明问题，展示例子或简短执行过程，再解释解决方案及其必要性。区分必需行为与可选复杂度。
- 在执行实现步骤前先回答信息性问题。独立的信息性提问保持讨论；已授权工作中的提问先回答，再在约定范围内继续。遵守明确的讨论或审批暂停点。
- 回应反馈或分析时，先说明赞同、部分赞同或反对及其理由，再描述改动。证据与用户的判断或方案冲突时，说明依据、影响和建议。区分事实错误与偏好取舍。出现新证据或条件变化，且足以影响既有决定时，再重新讨论。

## 执行与授权

- 所有文字使用 Sepia, 包括 Beads 记录, 保留事实和技术细节.
- 提出方案前，阅读相关实现、已确认决策和必要的官方文档，优先复用现有能力。
...
- 编辑文件前完整阅读该文件。大范围修改、调查或审计时，完整阅读相关文件，不依赖搜索片段。
- 仅在绝对必要时使用 `any`，不得借此绕过现有边界解码、类型保留或禁止 suppress 的要求。
- 根据已安装依赖的类型声明和版本核对外部 API，查阅对应的官方文档，不凭猜测使用。
- 所有导入均放在模块顶层，包括类型导入。不使用动态导入或 `import('package').Type` 这样的内联导入类型。
- 没有独立职责的单行、单调用点 helper 应内联。有价值的模块边界和不变量仍按现有抽象审查规则保留。
- 对过时依赖造成的类型不兼容，通过更新依赖解决，并遵循现有授权、审查及框架决策。不通过删除或降级有意提供的功能绕过这些错误。
- 删除看似有意保留的代码或功能前，明确其删除为何属于约定范围；仍不清楚时询问用户。已有任务授权不重复请求。
- 没有需求时，不为旧版本或旧调用方新增兼容代码。保留约定的支持范围和行为；接口或持久化格式变更继续遵循现有授权、兼容性及恢复规则。
- 通过源文件或生成器更新生成文件，不直接手改生成结果。
