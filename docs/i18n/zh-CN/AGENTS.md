...
- 提出方案前，阅读相关实现、已确认决策和必要的官方文档，优先复用现有能力。证据与用户的判断或方案冲突时，说明依据、影响和建议。区分事实错误与偏好取舍。出现新证据或条件变化，且足以影响既有决定时，再重新讨论。
...
- 仅在用户要求时创建 Pi Stuff TUI 原型。遵循仓库内 `prototype-tui` skill：明确并记录设计问题，按其逻辑／状态或 UI 工作流制作原型，再记录决策。宿主组件、主题和控件遵循 `design.md`，通过仓库内 tuistory skill 提供可交互的共享终端。使用隔离的示例数据，明确标注模拟状态。该流程不替代仓库验证，也不授权正式功能实现。
...
- 与 GitHub 协作时保留会话认领。未关联的 Issue 先导入、后认领；现有任务的反馈通过显式 Beads 更新对齐。不拉取覆盖现有任务，不做双向同步。发布前读取当前 GitHub 字段和评论，再用 `bd github push <bead-id>` 推送获准的任务字段。里程碑评论单独发布，记录 URL，重试前检查。报告待发布或发布失败，不声称存在无人值守同步。
...
- 用户要求的 TUI 原型: 仓库内 [prototype-tui skill](../../../.agents/skills/prototype-tui/SKILL.md) 及对应的 `LOGIC.md` 或 `UI.md` 分支，以及 [design.md](design.md) 和 [tuistory skill](../../../.agents/skills/tuistory/SKILL.md)。
