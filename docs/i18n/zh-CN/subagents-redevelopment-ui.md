...
状态: 2026-09-20 已接受 UIR01 和修订后的 UIR02; 2026-09-21 已接受 UIR03-UIR09. [正式规格](subagents-redevelopment-spec.md)在 [UI 审查](subagents-redevelopment-ui-review.md)后收束剩余原生组件、键盘和阅读细节. FleetView 只在列表获得键盘焦点时于列表上方显示帮助. 首版 UIR02 图片仍作为被否定的设计历史保留. 沿用 [#97](https://github.com/jczhang02/pi-stuff/issues/97), 属于 [#64](https://github.com/jczhang02/pi-stuff/issues/64). 运行范围已在[重新开发决策](subagents-redevelopment.md)中确定.

本记录保留已完成的真实场景 UI 讨论: 主界面与页面关系、FleetView 与任务结构、详情与可观测性、介入操作、完成与历史、键盘与视觉统一. 根据维护者反馈, 本轮同时梳理详情的信息层级. 先前 UI 作为起点, 不整套继承它所依赖的旧运行要求.
...
Transcript 进入完整记录. Info 提供配置、工作区、详细用量及历史的入口, 最终组织见正式规格.
...
FleetView 继续使用 UIR01 已接受的紧凑列表, 不因任务存在依赖而改变行布局. 独立任务无需额外关系图. 选中的任务属于依赖流程时, 提供入口, 在底部查看区域打开其真实依赖图. 正式规格将局部入口定为 `g`.
...
沿用 Pi 配置中的提交/换行按键, Esc 不发送并返回. Esc 或提交失败时保留草稿. 输入时字母属于编辑器, 只显示输入帮助. 新任务被接受后, 显示它真实的启动/工作详情. 按 RQ07 在历史中保留旧报告、状态、耗时和用量, 不覆盖旧记录, 也不将旧报告标成新任务的输出. 正式规格规定 Info 和历史阅读路径.
...
**UIR09 已接受.** 续聊时保留完成报告, 代码保存失败时突出错误并保留报告. 复用已接受的详情和局部输入区, 保留逐次任务记录, 展示真实 Git 结果.

## 审查与正式规格

UIR01-UIR09 全部接受. [UI 审查](subagents-redevelopment-ui-review.md)保留当前视觉方向, 通过[正式规格](subagents-redevelopment-spec.md)补齐剩余契约: Pi 原生组合方式、焦点优先级、完整 Transcript/Info/历史路径、有界布局及统一状态/指标语义. 上方历史按键、图片几何和提案措辞作为设计过程保留, 不再是待决事项.

已目视检查概念图, 核对 Pi/arhen 源码及 harness 参考. 尚未执行重新开发后的完成、续聊、恢复、提交失败或终端交互测试. 报告、测试结论、错误及指标仍为示意, ImageGen 概念图不作为终端验收证据.
