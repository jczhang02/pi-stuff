...
结构还小时先审查职责与导入边界; 等实际目录和依赖模式稳定后, 再自动化检查导入或循环依赖.

引入共享抽象前, 用保留当前拥有者或直接内联的简单方案比较同一行为. 只有在抽象能集中不变量或隔离已证实的变化来源时才保留; 否则保持简单形式和直接导入.

### 源码布局

- 根目录 `index.ts` 是唯一的 Pi 扩展入口, 直接读取包配置并注册各项能力.
- 功能实现放在 `src/<capability>/` 下, 新能力实现时添加为同级目录.
- `src/pi/` 管理共享宿主配置和工具开关策略. 功能专用的 Pi 注册, 认证和生命周期适配归所属能力管理.
- 源码模块提供能力接口, 不导入根入口. 包启动的组合逻辑放在根 `index.ts`, 不增加第二个入口或转出层.
- 能力内部结构应与实际实现规模相称. 文件和子目录承担具体职责时再创建.
- 修改布局或入口时, 在同一个 PR 中更新受影响的导入, 包元数据和中英文文档. 审查模块归属, 并验证真实 Pi 包加载.

## 源码修改

- 根据已安装依赖的类型声明、版本和对应官方文档核对外部 API。
- 所有导入均放在模块顶层，包括类型导入；不使用动态导入或 `import('package').Type` 这样的内联导入类型。
- 没有独立职责的单行、单调用点 helper 应内联。有价值的边界和不变量继续按抽象审查规则保留。
- 对过时依赖造成的类型不兼容，通过更新依赖解决，并遵循既有授权、审查和框架决策。保留有意提供的功能，不通过删除或降级功能绕过错误。
- 删除看似有意保留的代码或功能前，明确其删除为何属于约定范围；仍不清楚时才询问。复用已有任务授权。
- 仅在有需求时为旧版本或旧调用方添加兼容。保留约定的支持范围和行为；公共接口或持久化格式变更仍须遵循授权、兼容性和恢复要求。
- 通过源文件或生成器更新生成文件。
...
仅在绝对必要时使用 `any`，不得借此绕过边界解码、类型保留或下文的禁止 suppress 要求。
