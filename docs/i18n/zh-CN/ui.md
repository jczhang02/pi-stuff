...
针对性系统测试使用 Bun 1.4.0 下的 Pi 0.85.1 和编译 Pi 0.87.0, 配合隔离确定性 provider. 先前 reload/resume 失败已通过获批 TUI 查询修复: 原生工具保留执行定义, 缺失的 Web 历史仅获得 renderer. 禁用的 Web 工具仍不出现在模型请求中. 测试检查保留历史、不重跑工具、不改写会话. 完整配置/生命周期覆盖、最终独立审查、完整性能对比及真实模型长时间验收仍待完成.
