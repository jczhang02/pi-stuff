...
[已确认 UI 规格](https://github.com/jczhang02/pi-stuff/issues/106)正在实现. 本分支目前修改原生 Bash、Write、Edit、Read、Grep、Find 和 Ls 展示. Web 工具使用同一套检索展示. assistant 正文已在原生 Markdown 外添加独立前导点区域. Thoughts 已添加前导点和行内标签, 保留原生斜体 Markdown 与展开行为. 各块独立计时仍未完成. 原型仍是视觉参考, 不代表生产验收通过.
