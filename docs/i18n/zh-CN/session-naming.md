...
- **Naming rules** 使用 Pi 多行编辑器修改完整规则. `Restore defaults → Naming rules only` 恢复内置英文规则.
- **Maximum length** 接受正整数. `Restore defaults → Maximum length only` 恢复 80 码点.
- **Restore defaults** 选择全部设置或单独规则/长度默认值, 再确认重置, 保留当前名称.

配置提交成功会取消旧命名请求, 不重试. 保存失败则保留生效配置和未完成请求. 不覆盖外部文件修改, 需先 `/reload` 再重试. 已确认的设置会保存; 原生取消操作放弃未提交文本, 不撤销已经开始的保存. 会话替换或 reload 会等待已确认的保存结束, 再为新 runtime 加载设置.

面板使用 Pi 原生列表、搜索、文本对话框和确认选择器. AutoName 全部页面及尺寸提示均遵循原生取消绑定: 默认 Esc 或 Ctrl-C, 也可使用配置后的替代键. 规则直接打开原生多行编辑器, 保留外部编辑器快捷键. 保存失败时, 已提交草稿仍可编辑. 数值输入在标题显示当前值. 长名称保留 `[` / `]` 翻页; 模型搜索在说明区显示选中标识符. 最小尺寸仍为 56 列、24 行. AutoName 与 RTK 共用原生首页/设置组合和列表默认行为; RTK 报告及可执行路径校验保留既有专属界面. 不新增命名进度或保存成功提醒.
...
原生组件迁移覆盖逐项保存和按范围重置、多行草稿恢复、无效长度、外部修改冲突、保存与会话替换竞态、原生取消键重映射、模型搜索、持久化状态刷新及长名称分页. 下方当前截图来自编译版 Pi 0.87.1 和本地受控模型, 尺寸为 100×30、56×26 及 56×24. RTK 使用本地可执行 fixture 和人工统计. 命名算法及默认 prompt 未变, 截图不是新的真实模型质量样本.

原生迁移验证: 固定依赖宿主全库通过 217 项测试、1,322 个断言. 编译版 Pi 0.87.1 通过 36 项面板/保存/共用布局用例、252 个断言, 最终 RTK 重复切换回归另通过 1 项、4 个断言. 静态检查和独立标准/需求审查通过. 审查修复覆盖提交草稿保留、鼠标键盘焦点、尺寸提示的原生取消及保存后设置显示.
...
下方 RTK 界面由同一套共享组件渲染. 功能摘要各异, 标题、菜单列、原生列表和按键提示使用共同组合.
