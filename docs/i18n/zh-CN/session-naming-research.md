...
生成名称统一英文, 使用 `type: Action object`, 不带括号范围字段. 维护者优先要求最少 token 消耗, 并参考 Goose 的前三条用户消息方案. 收集三条消息后请求一次, 与每条消息后各请求一次成本不同; 开场触发点和调用次数仍待确认. 手动生成的输入、模型路由、失败处理及其余生命周期边界也需约定. 这些局部决策不构成实现授权.
