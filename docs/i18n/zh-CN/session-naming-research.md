...
生成名称统一英文, 使用 `type: Action object`, 不带括号范围字段. 维护者优先要求最少 token 消耗. Goose 的前三条用户消息方案只是设想, 未被选定; 没有约定三条消息门槛. 开场触发点和自动调用次数仍待确认.

自动生成失败或输出不合格后不重试, 保留默认显示, 用户可执行命令. 显式生成针对当前明确的主任务, 不固守开头任务, 也不罗列整场对话历史. 输入选择、模型路由及其余生命周期边界仍需约定. 这些局部决策不构成实现授权.
