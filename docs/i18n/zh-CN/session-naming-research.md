...
维护者阅读调研后要求继续设计访谈. 目前确认: 手动名称持续具有优先权, 直到用户明确恢复自动管理; 自动生成的名称统一使用英文. 单次显式重新生成是否同时恢复自动管理, 仍待决定.

具体英文命名格式尚未确定. 用户明确替换主任务后是否自动跟随改名也未确定, 维护者提出了隐形 token 消耗问题. 是否跟随目标与如何检测变化、消耗多少需要分别决定. 尚未批准逐轮分类、检查频率、模型路由或 token 预算. 这些局部决策不构成实现授权.
