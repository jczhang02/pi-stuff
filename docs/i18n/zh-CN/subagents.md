...
在 fullscreen Pi 中，鼠标滚轮按 Pi 原生步长滚动子代理记录；Page Up/Page Down 和
Home/End 也可翻阅。分页、半页、逐行和首尾跳转遵循配置的 Pi viewport 快捷键。
滚动保留编辑器草稿和 Fleet 焦点；`Ctrl+End` 沿用 Pi 原生的输入光标移至行尾行为。
`Ctrl+C` 返回主编辑器。子代理运行中按 Esc 会请求取消（正在
...
只有子代理 editor 的右上角边框显示名称和任务，主编辑器不显示 agent 标签。输入区域、
空输入光标、内边距及思考级别对应的边框颜色沿用 Pi 原生编辑器；不添加占位文字或
额外的提示符。

子代理查看器尚不是完整的原生 Pi 页面。fullscreen 搜索目前会搜索底下的主会话；
补全、按用户消息跳转、运行指示器和部分渲染设置也未完整接入。具体差异和待修问题见
[原生对照记录](subagent-verification.md#与原生-pi-0851-对照)，不能将其视为与主界面完全一致。
...
恢复时会重新检查允许的工具和原工作树元数据。已保存的可写子代理必须仍对应注册的独立
工作树。原对话文件必须存在，且会话头中的 id 与已保存记录一致；空文件、无效会话头或
不同会话 id 会被拒绝，不会在原位置创建新对话。消息记录沿用 Pi 原生解析器，会跳过
损坏的 JSONL 行；这里不做整个文件的完整性校验。备份或移动主会话时，需要一并保留
子会话文件和 sidecar。
