<!-- translation-source: docs/agents/issue-tracker.md; translation-source-sha256: 2b575f243b5329fd78d8ab4c4abdc2f492b9ee063746783ca256f21dc5e10ecf -->
...
- PR 交付以当前 PR head 为目标，接受 pull_request 或手动运行。发布器验证当前 workflow 的 `Plan`、`Checks`、`Tests`、`Verify`，不再重新按路径分类；只有成功的 Plan 明确要求零测试时，`Tests` 才可跳过。workflow 不完整或已过期时阻止发布。
- 仅分支交付以最后记录的提交为目标，接受 push 或手动运行，并要求同样的四个 job；允许同样有效的 no-tests 例外。未测试的功能分支需要手动触发 CI。
