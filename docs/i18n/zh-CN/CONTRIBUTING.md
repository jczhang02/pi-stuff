...
运行 `bun run test` 执行离线产品套件, CI 在现有 `checks` 作业内运行同一命令. 编译宿主的独立验证方式见[终端 E2E](quality-assurance.md#终端-e2e). 已删除治理程序的六组测试不作为占位保留. 自有行为确需维护时再增加测试, 包括有用的 bug 回归；报告实际命令和局限. 标准 hook 接线只做聚焦的集成验证, 不另建测试套件重测 Husky 或 commitlint.

作者和审查者直接核查受影响的 Markdown 链接、模板、标签、依赖和 CI 安全设置, 不再使用自研仓库政策或 PR 正文验证器. 发布证据前移除秘密和个人信息.

按[质量保证政策](quality-assurance.md)选择测试层级、隔离环境和必要验证. 离线测试通过不代表在线后端或未测试运行时已经兼容.
