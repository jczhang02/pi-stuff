...
| 区域                     | 旧版参考                                                                        | 本轮方向                                                                                           | 边界 / 待讨论                                                                                       |
| ------------------------ | ------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| 欢迎页                   | 带框 Pi logo、模型、目录、加载清单                                              | 保留旧 Pi Stuff 的带框欢迎页和启动信息.                                                            | 欢迎页形态在本次预览中固定. 会话身份/命名不在本轮范围内.                                            |
| 用户与 assistant 消息    | 用户卡片、Markdown、文件/代码/skill 样式                                        | 保留旧 transcript 前缀和阅读顺序: 用户 ``, assistant/tool `•`, 工具结果 `⎿`. 保留可读的 Markdown. | 优先使用原生渲染; 原生用户消息没有与自定义消息相同的任意替换接口.                                   |
...
| 工具详情                 | 旧版独立 `/tools` 检查器                                                        | 复用 Pi 原生单工具点击展开, 删除独立检查器. 保留会话内 `Tool(target)` 标题.                        | 已核对本地 Pi 0.85.1 `ToolExecutionComponent.createResultRegion`. 自定义消息样例不验证原生鼠标行为. |
| 图片、附件与可视输出     | 图片/媒体和 fenced visualization                                                | 内容留在所属消息中, 保留文字/路径退路.                                                             | 终端图片协议与截图导出需要单独用真实媒体验证.                                                       |
| 提示、错误与恢复         | 保留的诊断展示                                                                  | 会话中说明失败操作、原因、实际结果和下一步.                                                        | 测试失败不表示修改已回滚. 需要处理的错误不定时消失.                                                 |
...
本轮先讨论共享外框与设置, 再讨论会话与工具. 会话身份/命名、Goal、Todo、subagents、后台工作、BTW 和通知不在处理范围内, 本研究不为它们提出实现建议.
...
预览保留滚动会话和 Pi 的输入区. 欢迎页、正文、工具和 footer 使用同一活动主题. 工具块按动作/目标、结果、内容排列, 高度由有用内容决定, 不强制单行.

需要判断的是完整工作过程: 用户能否看到 agent 刚刚解释了什么、现在做什么、是否需要介入, 同时还能输入. 如果任务或状态指标挤占这些内容, 单独把 diff 做漂亮也不能解决问题.

样例围绕同一任务: 去掉跨页重复搜索结果, 保留顺序和游标. 每个场景是选取的阶段, 不是一次真实编码过程的录像. 测试数量、模型、费用、时长和加载清单都是示例数据. 扩展不调用 provider、真实 agent、测试命令, 也不修改仓库.

完整证据见[可运行原型](../../../../prototypes/ui-direction/README.md)和截图画廊. 工具密度、footer 字段、大 diff 导航仍待结合图片讨论. 生产入口不加载此原型.
