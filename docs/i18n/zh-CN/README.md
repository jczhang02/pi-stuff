...
维护范围为 Linux Bun 编译版 Pi `0.85.1` 至 `0.87.1`, 直接验收样本为 `0.85.1`、`0.86.1` 和 `0.87.1`. 新稳定版在验收前允许尝试. 实际宿主运行时、证据和维护政策见 [Pi 版本支持](pi-version-support.md). 开发技术栈为 TypeScript、Bun `1.4.0` 和 Effect v4 (`4.0.0-rc.112`), 纯算法保留为普通函数. 除网页访问、RTK 集成和主题外, 下方能力仍是产品方向, 不是已实现功能.
