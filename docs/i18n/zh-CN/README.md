...
[子代理](subagents.md)增加并行和依赖委派、定向消息、停止与恢复、已完成子代理续聊和逐次执行历史. FleetView 与底部检查界面保留主对话可见. 固定 SDK 和编译宿主的检查见[验收记录](subagents-redevelopment-acceptance.md).

包内还提供 Catppuccin、Tokyo Night、Gruvbox 和 Rosé Pine 共十款明暗[主题](themes.md). 通过 Pi 主题设置选择, 加载包会保留当前选择.

已测试目标是 Linux Bun 编译版 Pi `0.85.1`. 技术栈为 TypeScript、Bun `1.4.0` 和 Effect v4 (`4.0.0-rc.112`), 纯算法保留为普通函数. 其他宿主覆盖按能力分别记录. 除网页访问、RTK 集成、子代理和主题外, 下方能力仍是产品方向, 不是已实现功能.
...
- [src/subagent/](../../../src/subagent/) 管理委派、原生子会话、工作树、保留记录和检查界面.
...
| [子代理](subagents.md)                            | 委派、查看、干预、续聊和代码交接         |
