...
- **语义化 Session 名称**——首次由用户直接发起的工作 settled 后，会获得一个有界的模型生成名称；
  `/autoname` 可显式刷新，`/autoname settings` 可调整常用策略，而自动命名不会接管 Child Agent Session。
...
| `/autoname` · `/autoname settings` | 重新生成当前 Session 名称，或配置自动命名策略 |
...
| `session-naming` | 用户直接发起的工作 settled 后生成有界语义 Session 名称，维护分支内所有权状态，并提供 `/autoname` 控制 |
