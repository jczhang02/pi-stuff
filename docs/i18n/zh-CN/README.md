...
[会话命名](session-naming.md)只在开场生成一次名称, 支持手动 `/autoname` 替换, 可配置模型、风格和长度限制.

包内还提供 Catppuccin、Tokyo Night、Gruvbox 和 Rosé Pine 共十款明暗[主题](themes.md). 通过 Pi 主题设置选择, 加载包会保留当前选择.

已测试目标是 Linux Bun 编译版 Pi `0.85.1`. 技术栈为 TypeScript、Bun `1.4.0` 和 Effect v4 (`4.0.0-rc.112`), 纯算法保留为普通函数. 其他平台/宿主版本尚未验证. 除网页访问、RTK 集成、会话命名和主题外, 下方能力仍是产品方向, 不是已实现功能.
...
- [src/naming/](../../../src/naming/) 管理命名资格、请求限制和结果发布保护.
