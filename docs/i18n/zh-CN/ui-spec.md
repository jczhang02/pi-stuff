...
本文用于审阅会话 UI. 当前规则对应 `codex/ui-session-prototype` 分支的原型, 产品基线为 `cd0f174`, 使用 Pi 0.85.1 和 Bun 1.4.0. 生产实现尚未采纳, 待讨论的一项建议列在文末.
...
性能和复用 Pi 优先于原型逐像素一致. 还原某种样式若需要不成比例的开销, 可以保留原生展示. 验收同时覆盖输入、滚动、流式输出、展开和缩放的流畅度, 以及相同工作负载下启用新 UI 前后的耗时与内存回归. 长会话不能无界累积 UI 开销. 具体场景和阈值尚待确定. 优先复用 Pi 能力, 保持代码简洁. 生产架构和实现仍在讨论, 原型认可不等于这些决策已确定.

当前实施范围只改 pi-stuff. 优先使用 Pi 公开扩展接口; monkey patch 仅在出现具体问题后讨论, 不预先批准或禁止某种 patch. 新 UI 严格只改变展示, 不改变工具执行、模型收到的结果或会话保存的记录. 旧会话也应用新展示规则, 不改写历史, 缺失字段省略. assistant 以列表开头时前导点是否保留, 记录为正式实现后的真实 E2E 验证项, 当前不额外调查.

实现选型先查 Pi 和同类包, 只将仍需产品取舍的问题带入讨论. 参考 [pi-tool-display 0.5.0, 91cef758](https://github.com/MasuRii/pi-tool-display/tree/91cef7580078371f8dc49a8607222807ad6a424d); 参考实现不代表批准安装依赖或整体移植.

有实际用途的展示选项尽量可配置, 默认值沿用已认可的原型. 配置文件提供完整选项, 交互设置入口提供常用项, 包括预览行数、检索聚合、diff 展示及各类 UI 开关. 已有主题、Hide thinking 等设置继续复用 Pi. 配置仅作用于全局, 不提供项目级覆盖. 具体字段尚未确定.

Pi 内置工具和 pi-stuff 自有工具使用新展示. 第三方工具已有自定义渲染时默认保留, 提供逐工具的明确接管选项, 开启后才使用通用样式.

检索聚合保留为首版实施目标. 缺少直接公开分组接口不构成提前删减的理由. 若真实测试证实明显性能损耗, 如交互卡顿或 resume 载入会话显著变慢, 再根据具体测量结果讨论取舍.
...
工具使用状态点、Action(target), 下一层以 ⎿ 引出结果. 每个可见子结果摘要一个连接符, 摘要续行与摘要正文对齐, 结果正文保留自己的内容缩进. 一次调用可以有多个子块: 结果、可选的已配置 timeout、上游截断/日志提示. 没有元数据就不增加行. Web 技术元数据仍只在展开时显示. 展开的独立工具各有自己的结果连接符. 这是两层语义结构, 不限制为两行终端文字. 收起标题最多两行, 仅在还有标题文本时显示省略号; 展开显示完整目标. 代码保留语法高亮. 成功、运行、失败、取消同时使用语义颜色和文字.
...
Edit 使用统一 diff 与单列行号: 删除用旧行号, 新增及上下文用新行号. +/- 标记和增删底色与语法颜色共同表达改动. 新旧源码分别高亮. 收起显示已有 hunk 的前六行渲染结果, 包含未变上下文. 宽屏中的小样例可完整显示函数、改动与闭合行. 展开恢复全部剩余行与完整目标. 折行不重复行号或增删符号. Write 默认显示路径、写入行数与前三行渲染后的高亮正文. 短内容完整显示, 展开后显示其余内容. Diff 行由样例指定.
...
工具和检索组不显示 `· expand` 或 `· collapse` 后缀. 收起工具确实隐藏了已保留的渲染正文行时, 在预览下方显示 `n more lines`, 与结果摘要分开. 只统计正文行, 不计标题折行或元数据. 正文已完整显示或为空时不显示数量. 即使两种工具视图完全相同, 原生点击和 Ctrl+O 仍可切换. 检索摘要无后缀, 仍可点击.
...
[剩余建议依据](research/claude-code-ui-details-2026-09-22.md) · [已采纳的子块与展开研究](research/claude-code-default-tui-2026-09-22.md)
