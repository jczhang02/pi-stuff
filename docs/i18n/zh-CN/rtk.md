...
面板保存成功后立即影响后续操作, 并跨重启保留. 面板打开期间发生外部配置修改时, 保存会报告冲突, 不覆盖新修改; 使用 `/reload` 后重试. 写入使用原子替换, 保留无关设置和现有符号链接. 保存被中断可能在解析后的配置文件旁留下 `.lock` 文件. 手动删除陈旧锁前, 先确认没有 Pi Stuff 保存操作在运行, 然后 reload. 不要删除其他会话正在使用的锁.
