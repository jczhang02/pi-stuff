...
- The implementation package is Aggregate-only support code, not a separately selectable Capability Package: a
  correct registry can exist only while the Aggregate owns registration. Its manifest declares this exception
  explicitly instead of exposing a non-functional standalone Pi extension.
