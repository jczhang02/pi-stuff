...
- Each Bash call renders once as `Running <command>` while active and `Ran <command>` after settlement, followed by a child output preview. The command preview follows Claude Code's two-source-line and 160-code-unit cap without parsing `&&`, pipes, semicolons, loops, or other shell syntax into inferred subcommands. The first child row uses U+23BF `⎿`; later lines align to the child text origin. The compact output shows three physical lines, admits a fourth instead of replacing it with a `+1 line` hint, and otherwise reports hidden lines with the global `ctrl+o to expand` action. Empty output, streaming updates, stderr, exit status, timeout, cancellation, rejection, and failures remain explicit. This is the deliberate Bash exception to complete grouping; Bash never contributes a duplicate `N commands` aggregate clause.
...
Representative settled clauses are `Completed goal`, `Reported goal blocker`, `Committed cf12251`, `Pushed to main`, `Created PR #42`, `Generated 2 images`, `Changed 4 files`, `Updated 3 tasks`, `Ran 2 agents`, `Launched 3 background tasks`, `Invoked 2 MCP tools`, `Searched 3 patterns`, `Searched 2 web queries`, `Fetched 4 pages`, `Read 5 files`, `Listed 2 directories`, `Viewed 1 image`, `Searched history`, `Reviewed 2 history ranges`, and `Checked 6 tasks`. Bash is represented by its standalone `Ran <command>` blocks instead. Present-tense forms replace settled aggregate forms while active. Singular/plural grammar and clause capitalization follow Claude Code's sentence style.
