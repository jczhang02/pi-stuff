...
- Every compact Group carries a dim `ctrl+o to expand` hint. Active Groups use the animated Tool marker. Settled success Groups have no colored bullet and use dim text; settled failures use a red marker, while rejection or cancellation without failure uses yellow.
