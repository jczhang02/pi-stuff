...
- Every non-Bash Tool type participates, including consequential, Agent, backgrounded, failed, rejected, and cancelled calls. Bash uses the standalone operation grammar below.
...
- Each Bash call renders once as `Bash(<command>)` in both active and settled states, followed by a child output preview. An active call with no streamed output shows `Running…`; a settled empty result shows `(No output)`. The command preview follows Claude Code's two-source-line and 160-code-unit cap without parsing `&&`, pipes, semicolons, loops, or other shell syntax into inferred subcommands. The first child row uses U+23BF `⎿` with Claude's child-text origin; later lines align to that origin. The compact output shows three physical lines and otherwise reports hidden lines as `… +N lines (ctrl+o to expand)`. Streaming output, stderr, exit status, timeout, cancellation, rejection, and failures remain explicit. This is the deliberate Bash exception to complete grouping; Bash never contributes a duplicate `N commands` aggregate clause. The accepted Pi Stuff transcript marker remains `•`; operation wording, row count, spacing, and child hierarchy are certified against matched real Claude Code 2.1.220 and Pi Host 0.84.1 captures.
...
Representative settled clauses are `Completed goal`, `Reported goal blocker`, `Committed cf12251`, `Pushed to main`, `Created PR #42`, `Generated 2 images`, `Changed 4 files`, `Updated 3 tasks`, `Ran 2 agents`, `Launched 3 background tasks`, `Invoked 2 MCP tools`, `Searched 3 patterns`, `Searched 2 web queries`, `Fetched 4 pages`, `Read 5 files`, `Listed 2 directories`, `Viewed 1 image`, `Searched history`, `Reviewed 2 history ranges`, and `Checked 6 tasks`. Bash is represented by its standalone `Bash(<command>)` blocks instead. Present-tense forms replace settled aggregate forms while active. Singular/plural grammar and clause capitalization follow Claude Code's sentence style.
...
`Ctrl+O` restores the original controlled non-Bash Tool rows and their bounded result renderers globally. Standalone Bash remains one operation block while its command and output expand in place. `/tools` restores one selected group's complete member list and bounded per-member text. Neither detail path changes model-visible Tool results or persisted session entries.
...
3. Compact aggregate mode contains no raw command, full path, Tool name list, trailing `done`, member argument block, or textual member result. Standalone Bash blocks are the explicit exception: they retain one bounded command title and output preview. Semantic clauses, issue disclosure, interactive permission UI, and real media are preserved.
