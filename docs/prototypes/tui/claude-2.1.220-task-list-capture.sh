...
if [[ $($claude_bin --version) != "2.1.220 (Claude Code)" ]]; then
	echo "Task-list reference capture requires Claude Code 2.1.220" >&2
	exit 1
fi
...
mock="$repo_root/docs/prototypes/tui/claude-2.1.220-task-list-mock.ts"
...
tmux_session="pi-stuff-claude-21220-task-list-$$"
command=""
printf -v command \
	'env -i HOME=%q XDG_CONFIG_HOME=%q XDG_CACHE_HOME=%q XDG_DATA_HOME=%q CLAUDE_CONFIG_DIR=%q ANTHROPIC_BASE_URL=%q ANTHROPIC_API_KEY=%q HTTP_PROXY=%q HTTPS_PROXY=%q ALL_PROXY=%q NO_PROXY=%q no_proxy=%q PATH=%q LANG=%q TERM=%q SHELL=%q CLAUDE_CODE_ENABLE_TASKS=true CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC=1 CLAUDE_CODE_DISABLE_TERMINAL_TITLE=1 DISABLE_TELEMETRY=1 DISABLE_AUTOUPDATER=1 DISABLE_ERROR_REPORTING=1 %q --no-chrome --disable-slash-commands --model %q --tools default --permission-mode dontAsk' \
	"$home_dir" \
...
tmux send-keys -t "$tmux_session" -l "Create and work through the deterministic seven-item task list."
tmux send-keys -t "$tmux_session" Enter
wait_for_event "hold-running"
wait_for_visible_text "… +2 pending"
wait_for_visible_text "ctrl+t to hide tasks"
capture_frame "claude-2.1.220-task-list-working"

tmux send-keys -t "$tmux_session" C-t
wait_for_visible_text "ctrl+t to show tasks"
reject_visible_text "… +2 pending"
capture_frame "claude-2.1.220-task-list-hidden"

tmux send-keys -t "$tmux_session" C-t
wait_for_visible_text "ctrl+t to hide tasks"
wait_for_visible_text "… +2 pending"
capture_frame "claude-2.1.220-task-list-restored"

wait_for_event "hold-mixed"
wait_for_visible_text "✔ Inspect task list placement"
wait_for_visible_text "◼ Compare visible row density"
capture_frame "claude-2.1.220-task-list-mixed"

wait_for_event "hold-complete"
wait_for_visible_text "Task-list probe complete."
reject_visible_text "… +2 pending"
reject_visible_text "ctrl+t to hide tasks"
capture_frame "claude-2.1.220-task-list-completed-idle"

for expected_event in create-seven first-running hold-running mixed-update hold-mixed complete-all hold-complete; do
	if [[ $(rg -c -- "\"kind\":\"$expected_event\"" "$event_log") -ne 1 ]]; then
		echo "Expected exactly one fixture event: $expected_event" >&2
		exit 1
	fi
done
if ! rg -F --quiet -- '"TaskCreate"' "$event_log"; then
	echo "TaskCreate was not enabled in the released Claude Code request" >&2
	exit 1
fi
if rg -F --quiet -- 'missing-task-identifiers' "$event_log"; then
	echo "Released Claude Code did not return the expected synthetic task IDs" >&2
	exit 1
fi

for state in working hidden restored mixed completed-idle; do
	artifact="$artifact_dir/claude-2.1.220-task-list-$state.png"
	if [[ ! -s $artifact ]]; then
