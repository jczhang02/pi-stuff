...
- [ ] 以 Beads 为唯一正式任务记录；代码开始前，修正与当前决策不一致的旧任务。
...
- [ ] 做成已经确认的两行样式，视觉参考 `pi-footer`，但继续使用 Pi Stuff 自己的组件体系。
- [ ] 第一行按这个顺序显示：模型 → Thinking 强度 → Fast（仅开启时）→ 当前目录 → Git → Context → 缓存命中率 → Cost 或 Codex weekly limit。
- [ ] 不显示 token 数，不显示 worktime，Fast 未开启时不留空位。
- [ ] 第二行只显示上一次用户输入的 Prompt，前面使用紧凑的蓝色实心圆点。
- [ ] 两行的图标槽位和文字起始位置必须视觉对齐；同时修正 Nerd Font、宽字符和中文造成的偏移。
- [ ] 图标大小保持一致，避免某些图标显得特别大。
- [ ] 窄屏时整段隐藏低优先级信息，不能把字段截成难懂的残片，也不能发生重叠。
- [ ] 使用真实 PTY 截图检查普通宽度、窄屏、中文 Prompt、Session 恢复和窗口缩放。

### 1.2 Fleetview 的位置和帮助行（`ps-5cb.7.8.5`）

- [ ] Fleetview 永远位于两行 Statusline 下方，成为 Pi Stuff 最底部的区域。
- [ ] 删除常驻的 `↓ to manage`。
- [ ] Fleetview 自己保留一行固定高度的帮助槽位：静止时是空白行，进入管理时在原位替换成 `↑/↓ select · Enter view · x stop · Esc return`。
- [ ] 窄屏时缩短整条帮助文案，不出现 `tx… done` 这类把不同字段挤在一起的内容。
- [ ] 不在 Statusline 与 Fleetview 之间额外添加分隔线或空白；那一行空白属于 Fleetview 自己。
- [ ] 验证 Agent 启动、完成、失败、停止、窗口缩放和打开 Dialog 时都不会闪烁、重叠或残留旧行。

### 1.3 Welcome（`ps-5cb.7.8.6`）

- [ ] 保留已经确认的 Claude Code 风格 Welcome 卡片。
- [ ] 把目前的 Pi 图案换成 pi.dev 官方标识的终端 ASCII 重建版。
- [ ] 普通宽度显示完整 8×4 图案；空间不足时使用完整 4×2 图案，而不是把大图硬裁掉。
- [ ] 图案下方保留一行有效空白，再显示模型信息，避免图案和文字贴得太近。
- [ ] 图案使用 Pi 主题强调色；Pi Stuff 仍作为独立卡片标题，不冒充 Pi 本身。
- [ ] 用真实终端截图检查普通宽度、窄屏和不同主题下的效果。
...
- [ ] 长时间运行的测试、构建和开发服务器可以转入后台，主 Agent 无需等待，可以继续工作和回复用户。
- [ ] 当前台 Bash 正在运行时，允许用 `Ctrl+B` 把它转到后台；其他时候不抢占这个快捷键。
- [ ] 后台任务保留有上限的输出，不能让日志无限增长。
- [ ] 退出 Pi 时终止本 Session 启动的整个进程树；先发 TERM，必要时再发 KILL。
- [ ] 防止 PID 复用、旧任务误认、孤儿进程和崩溃后的脏状态。
- [ ] 不增加任何 Permission 弹窗或确认提醒。

### 2.2 Monitor

- [ ] Agent 可以等待一个明确事件，而不是反复占用主对话轮询。
- [ ] 支持等待命令、日志、文件或 HTTP 端点中的已知结果，例如 `ready`、`ERROR`、CI 完成或服务可访问。
- [ ] 事件发生、超时或失败后，在对话中留下简洁结果，并让主 Agent 继续处理。

### 2.3 `/tasks`

- [ ] `/tasks` 打开一个全宽、非浮动的 Command Dialog。
- [ ] 它只管理当前 Session 的实时后台活动：Background Shell、Monitor 和正在运行的 Subagents。
- [ ] `/agents` 继续负责 Agent 详情；Todo、Goal 和 Beads 保持各自职责，不合并进 `/tasks`。
- [ ] 列表和详情可查看状态、运行时间、最近输出，也可以停止任务并返回主对话。
- [ ] 平时不在 Statusline 或对话底部常驻显示任务面板；任务完成后只留下紧凑结果。

### 2.4 实现来源

- [ ] 重新比较成熟 Pi Package，至少验证 `pi-patty-bg-tasks` 和 `pi-background-tasks` 的许可证、质量、维护情况和 Pi 兼容性。
- [ ] 不因为某个 Package 借鉴 Claude Code 就排除它；只看最终能力和实现质量。
- [ ] 采用成熟实现后必须 fork，但 fork 的源码只能进入 `pi-stuff` Monorepo，不能再建立独立本地仓库或 GitHub 仓库。

## 3. 把 Magic Context 调整为可替换的正式集成（`ps-5cb.21`）

- [ ] Pi Stuff 不再保存或维护 Magic Context Core 源码，也不维护独立 Magic Context fork。
- [ ] 使用经过审核的、固定版本的官方 `@cortexkit/pi-magic-context` Package。
- [ ] 官方 Package 放在现有 `pi-stuff-context` Adapter 后面；以后要替换上下文方案时，不影响 Suite 其他部分。
...
- [ ] 所有验证通过以后，才删除现有 Magic Context fork、工作目录和相关 GitHub 仓库。

## 4. 最终只保留一个 `pi-stuff` 仓库

- [ ] 唯一正式源码 checkout 是当前 `pi-stuff` 仓库，不保留任何持久的兄弟源码目录。
- [ ] 唯一正式 GitHub 仓库是 `jczhang02/pi-stuff`。
- [ ] Web 和 MCP 不再依赖自己账号下的 GitHub Release tarball；优先改用固定版本的官方 Package，并继续放在 Pi Stuff Adapter 后面。
- [ ] 如果某个能力确实必须修改上游源码，fork 直接作为 `pi-stuff` 内部 Package 保存，不创建新的仓库。
- [ ] Codex、Goal 和 RTK 已在 Monorepo 中，确认来源和行为都保留后，删除对应的临时 fork。
- [ ] 清理所有临时 worktree、额外本地源码仓库和多余的个人 GitHub fork。
- [ ] 删除前逐项确认功能、许可证、来源记录和测试都已迁移；不能先删再补。
- [ ] 保留 Settings Layer 中的 `packages/pi-stuff-current`。它只是指向当前安装版本的稳定链接，不是第二个源码仓库。

## 5. 对已经实现的能力做一次完整回归

这些能力原则上不重做，但必须确认真的能用、好用，并能在 Session 恢复后保持同样效果。

- [ ] Tool UI：运行中、成功、失败、展开详情、窄屏，以及退出后 Resume 都使用统一渲染，不退回 Raw 样式。
- [ ] Thinking UI：只显示当前实时 Thinking，原位更新，不把多个 Thinking 合并成一行，也不留下多余空白。
- [ ] Todo：首行和 Agent 输出对齐，任务行距离紧凑，快捷键和显示上限正常，Session 恢复正常。
- [ ] BTW：保持已经确认的简洁 Dialog，主任务继续运行，回答后恢复正确。
- [ ] Agents/Fleetview：Subagent 启动、并发、完成、失败、停止、进入详情和返回都正确。
- [ ] Goal：创建、持续执行、完成和恢复都正确。
- [ ] Web 与 MCP：连接、调用、错误退化和 Tool UI 正确。
- [ ] `/codex`：Fast 开关、Usage、Image Generation、Apply Patch 和 View Image 正确。
- [ ] RTK：在长输出场景下有效减少上下文占用，不破坏命令结果。
- [ ] `/ui`：只设置显示相关选项，不变成所有 Pi Stuff 功能的总设置页。
- [ ] 确认 Permission Capability 已完全移除，不再出现权限提醒。
...
- [ ] 主动制造命令失败、网络失败、Context 存储失败、Agent 失败和强制停止，确认退化方式清楚且不损坏 Session。
- [ ] 对发现的问题立即修复并重复真实 TUI 测试，直到日常使用没有明显不适。
