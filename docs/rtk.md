...
A successful panel save applies to subsequent operations immediately and survives restart. Changes made by another editor while the panel is open cause a conflict instead of being overwritten; use `/reload` before retrying. Saving replaces the configuration atomically and preserves unrelated settings and existing symlinks. An interrupted save can leave a `.lock` file beside the resolved configuration file. Check that no Pi Stuff save is active before manually removing that stale lock, then reload. Do not remove another running session's lock.
...
`/rtk` opens an inline list with Settings, Usage and Diagnostics. It uses Pi's theme, lists and selection keys. Escape returns from a detail page to the list, then restores the editor. The RTK extension adds no bottom status bar. A check beside the version means the executable probe succeeded; runtime details retain a textual status.
...
Usage supports Overview, Daily, Weekly, Monthly, History and Failures. Press `r` to refresh or retry. Returning from a temporary query cancels it. A failed query does not disable rewriting. Narrow layouts support 56 columns by 26 rows; smaller terminals show a size notice and retain an exit action.
