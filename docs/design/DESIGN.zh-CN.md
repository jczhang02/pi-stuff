...
不能看起来像两张并排的卡片。当任意一栏已经无法正常阅读时，应切换为单栏。当前栏的焦点通过栏标题的
强调色表达，不要在标题旁边再加一根只占一行的短竖线。
...
层级通过一条结构分隔线、克制的间距、语义对比和板块图标表达。不要给每个板块再套一个框。可滚动的
Welcome 身份卡是目前唯一确认可以使用完整边框的例外，因为它属于对话文档，不属于临时 Dialog 系统。

## 形状

Pi Stuff 的形状语言来自终端，以直线结构为主。Dialog 的结构线使用粗体框线字符；宽屏双栏在一条连续
的顶部粗线下使用一条粗体中间分隔线。小型 `◆` 只用于标记板块标题，不能表示生命周期状态、焦点或
Conversation Transcript 事件。
...
板块标题使用 `◆`，例如 `◆ Task`、`◆ Activity`、`◆ Output` 或 `◆ Details`。每个界面根据自己的
...
终端足够宽，而且用户需要反复对照同级条目的列表与详情时，一个 Dialog 可以同时显示两者。这条规则
适用于 `/tools`、`/tasks`、`/diagnostics` 和裸 `/btw` 的历史浏览。它们仍然是一块 Dialog：顶部只有一条
连续粗线，中间只有一条粗体竖向分隔线，切换选中项时外部尺寸不变。当前栏标题使用强调色表示焦点；
不要把 `◆`、`›` 或短 `│` 拿来表示栏焦点。

`/tools` 保持已经接受的固定 18 行。`/tasks` 只在右栏显示 Shell 或 Monitor 详情；Agent 行仍然进入单栏
`/agents`，不能嵌入第二份 Agent 详情。裸 `/btw` 可以把历史与已经完成的答案并列；`/btw <question>` 和
流式答案仍然使用单栏。`/agents`、`/ctx`、`/mcp`、`/rtk` 在任何宽度下都保持单栏。任何双栏都要在其中
一栏无法正常阅读之前切回原有的单栏列表与详情流程。
