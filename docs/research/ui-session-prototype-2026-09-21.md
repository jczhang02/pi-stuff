...
`verify-foreground.ts` runs the real foreground launcher inside an outer private PTY and checks its raw transcript through clean exit: inherited theme, explicit light, explicit dark, and interrupted replay. It rejects palette setters and resets while allowing queries. All four paths passed after the fix. This proves the outgoing protocol boundary, not native compositor behavior.
