...
## Direct Claude execution follow-up

[Claude Code 2.1.261 execution study](claude-code-result-marker-2026-09-21.md) now records 20 real-client runs and 43 terminal captures. It establishes the tested marker rules directly and distinguishes classic/fullscreen, collapsed/expanded, running and interrupted states. The marker can introduce a command preview or interruption annotation, so "child-content connector" is more accurate than an exclusively completed-result marker. The prototype remains a selected old-Pi/classic-inspired design, including its explicit differences from Claude.
