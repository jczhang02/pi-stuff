// Throwaway real-Pi UI host. Model replies are deterministic local fixtures.
import {
...
async function runHost(directory: string, scenario: string) {
  ...
  const factory: CreateAgentSessionRuntimeFactory = async options => {
    ...
    if (!model) throw new Error('The local main model is not registered.');
...
const program = Effect.gen(function* () {
  ...
  const run = Effect.tryPromise({
    try: async () => {
      ...
      await runHost(directory, scenario);
    },
